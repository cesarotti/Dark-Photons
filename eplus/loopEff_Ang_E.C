...
int loopEff_Ang_E(){
  ...
  const double angle_high = 5;
  const double angle_step = 1;

  const double ene_low = 1;
  const double ene_high = 5;
  const double ene_step = 1;

  const double brem_eff = 0.1;

  int h_ene_bin = (int (ene_high-ene_low)/ene_step) + 1;
  double h_ene_low = ene_low - ene_step/2.0;
  double h_ene_high = ene_high + ene_step/2.0;

  int h_angle_bin = (int (angle_high-angle_low)/angle_step);

  TH2D *h2D_ene_ang_eff = new TH2D("h1","ene_ang_eff", h_ene_bin, h_ene_low, h_ene_high, h_angle_bin, angle_low, angle_high); 

  for (double ang=angle_low; ang<angle_high; ang+=angle_step){
    for (double ene=ene_low; ene<=ene_high; ene+=ene_step){
      ...
      macfile << "/run/beamOn 0\n";
      macfile << "/gps/ene/min " << ene - 0.05 << " GeV\n";
      macfile << "/gps/ene/max " << ene << " GeV\n";
      macfile << "/brem/setLowTheta " << ang << " deg\n";
      macfile << "/brem/setHighTheta " << ang+angle_step<< " deg\n";
      ...
      h2D_ene_ang_eff->Fill(ene, ang+angle_step/2, 1-h2DPositron_PosC_0->GetBinContent(i) );

      data->Close();
    }
  }
  TCanvas *c1 = new TCanvas("c1", "ene_ang_eff", 800, 800);
  c1->cd();
  h2D_ene_ang_eff->GetXaxis()->SetTitle("Energy/GeV");
  h2D_ene_ang_eff->GetYaxis()->SetTitle("Angle/degree");
  h2D_ene_ang_eff->Draw("COLZ");
  h2D_ene_ang_eff->Draw("TEXT0 SAME");
  return 0;
}
