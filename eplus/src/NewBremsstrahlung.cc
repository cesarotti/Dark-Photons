...
// ********************************************************************
//
// $Id: NewBremsstrahlung.cc 75582 2013-11-04 12:13:01Z gcosmo $
//
// -------------------------------------------------------------------
//
// GEANT4 Class file
...
// 15-01-07 use SetEmModel() from NewEnergyLossProcess (mma)
...
NewBremsstrahlung::~NewBremsstrahlung()
{}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....

G4bool NewBremsstrahlung::IsApplicable(const G4ParticleDefinition& p)
{
...
void 
NewBremsstrahlung::InitialiseEnergyLossProcess(const G4ParticleDefinition*,
					       const G4ParticleDefinition*)
{
  if(!isInitialised) {

    //    if (!EmModel(1)) { SetEmModel(new NewBremsstrahlungModel(), 1); }
    if (!EmModel(1)) { SetEmModel(new G4SeltzerBergerModel(), 1); }
    if (!EmModel(2)) { SetEmModel(new NewBremsstrahlungRelModel(), 2); }
...
void NewBremsstrahlung::PrintInfo()
{
  if(EmModel(1)) {
    G4LossTableManager* man = G4LossTableManager::Instance(); 
    G4double eth = man->BremsstrahlungTh(); 
