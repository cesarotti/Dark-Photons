...
// ********************************************************************
//
// $Id: NewEnergyLossProcess.cc 79268 2014-02-20 16:46:31Z gcosmo $
//
// -------------------------------------------------------------------
//
// GEANT4 Class file
...
NewEnergyLossProcess::NewEnergyLossProcess(const G4String& name, 
					   G4ProcessType type): 
...
{
  /*
  G4cout << "** NewEnergyLossProcess::~NewEnergyLossProcess() for " 
	 << GetProcessName() 
...
void NewEnergyLossProcess::Clean()
{
  /*
  if(1 < verboseLevel) { 
    G4cout << "NewEnergyLossProcess::Clear() for " << GetProcessName() 
	   << G4endl;
  }
  */
...
G4double NewEnergyLossProcess::MinPrimaryEnergy(const G4ParticleDefinition*, 
						const G4Material*, 
						G4double cut)
{
  return cut;
...
void NewEnergyLossProcess::AddEmModel(G4int order, G4VEmModel* p, 
...
void NewEnergyLossProcess::UpdateEmModel(const G4String& nam, 
...
void NewEnergyLossProcess::SetEmModel(G4VEmModel* p, G4int index)
{
...
G4VEmModel* NewEnergyLossProcess::EmModel(G4int index) const
{
  G4VEmModel* p = 0;
...
G4VEmModel* NewEnergyLossProcess::GetModelByIndex(G4int idx, G4bool ver) const
{
...
G4int NewEnergyLossProcess::NumberOfModels() const
{
...
void 
NewEnergyLossProcess::PreparePhysicsTable(const G4ParticleDefinition& part)
{
  if(1 < verboseLevel) {
    G4cout << "NewEnergyLossProcess::PreparePhysicsTable for "
  ...
  if( particle != &part ) {
    ...
    if(1 < verboseLevel) {
      G4cout << "### NewEnergyLossProcess::PreparePhysicsTable()"
	     << " interrupted for "
  ...
  if(1 < verboseLevel) {
    G4cout << "NewEnergyLossProcess::PrepearPhysicsTable() is done "
...
void NewEnergyLossProcess::BuildPhysicsTable(const G4ParticleDefinition& part)
{
  G4bool verb =  false;
  if(1 < verboseLevel || verb) {
  
    //if(1 < verboseLevel) {
    G4cout << "### NewEnergyLossProcess::BuildPhysicsTable() for "
           << GetProcessName()
  ...
  G4bool master = true;
  const NewEnergyLossProcess* masterProcess = 
    static_cast<const NewEnergyLossProcess*>(GetMasterProcess());
  ...
  //if(1 < verboseLevel || verb) {
  if(1 < verboseLevel) {
    G4cout << "### NewEnergyLossProcess::BuildPhysicsTable() done for "
           << GetProcessName()
           << " and particle " << part.GetParticleName();
...
G4PhysicsTable* NewEnergyLossProcess::BuildDEDXTable(G4EmTableType tType)
{
  G4bool verb = false;
  if(1 < verboseLevel || verb) {
    G4cout << "NewEnergyLossProcess::BuildDEDXTable() of type " << tType
	   << " for " << GetProcessName()
  ...
  if(fTotal == tType) {
    ...
    G4cout << "NewEnergyLossProcess::BuildDEDXTable WARNING: wrong type "
	   << tType << G4endl;
  }
  ...
  for(size_t i=0; i<numOfCouples; ++i) {
    ...
    if(1 < verboseLevel || verb) {
      G4cout << "NewEnergyLossProcess::BuildDEDXVector Idx= " << i 
  ...
  if(1 < verboseLevel || verb) {
    G4cout << "NewEnergyLossProcess::BuildDEDXTable(): table is built for "
...
G4PhysicsTable* NewEnergyLossProcess::BuildLambdaTable(G4EmTableType tType)
{
  G4PhysicsTable* table = 0;
  ...
  if(fRestricted == tType) {
    ...
    G4cout << "NewEnergyLossProcess::BuildLambdaTable WARNING: wrong type "
	   << tType << G4endl;
  }

  if(1 < verboseLevel) {
    G4cout << "NewEnergyLossProcess::BuildLambdaTable() of type "
	   << tType << " for process "
...
void 
NewEnergyLossProcess::PrintInfoDefinition(const G4ParticleDefinition& part)
{
  if(0 < verboseLevel) {
...
void NewEnergyLossProcess::ActivateSubCutoff(G4bool val, const G4Region* r)
{
...
void NewEnergyLossProcess::StartTracking(G4Track* track)
{
  /*    
...
G4double NewEnergyLossProcess::AlongStepGetPhysicalInteractionLength(
...
G4double NewEnergyLossProcess::PostStepGetPhysicalInteractionLength(
                             const G4Track& track,
                             ...
                             G4ForceCondition* condition)
{
  ...
  *condition = NotForced;
  ...
  if (verboseLevel>2){
    //  if(particle->GetPDGMass() > 0.9*GeV){
    G4cout << "NewEnergyLossProcess::PostStepGetPhysicalInteractionLength ";
...
G4VParticleChange* NewEnergyLossProcess::AlongStepDoIt(const G4Track& track,
                                                       const G4Step& step)
{
...
void 
NewEnergyLossProcess::FillSecondariesAlongStep(G4double&, G4double& weight)
{
...
G4double 
NewEnergyLossProcess::SampleSubCutSecondaries(std::vector<G4Track*>& tracks, 
					      const G4Step& step, 
					      G4VEmModel* model,
					      G4int idx) 
{
...
G4VParticleChange* NewEnergyLossProcess::PostStepDoIt(const G4Track& track,
                                                      const G4Step& step)
{
...
G4bool NewEnergyLossProcess::StorePhysicsTable(
...
G4bool 
NewEnergyLossProcess::RetrievePhysicsTable(const G4ParticleDefinition* part, 
					   ...
					   G4bool ascii)
{
  ...
  if(1 < verboseLevel) {
    G4cout << "NewEnergyLossProcess::RetrievePhysicsTable() for "
...
G4bool NewEnergyLossProcess::StoreTable(const G4ParticleDefinition* part, 
...
G4bool 
NewEnergyLossProcess::RetrieveTable(const G4ParticleDefinition* part, 
...
G4double NewEnergyLossProcess::GetDEDXDispersion(
                                  const G4MaterialCutsCouple *couple,
...
G4double NewEnergyLossProcess::CrossSectionPerVolume(
...
G4double NewEnergyLossProcess::MeanFreePath(const G4Track& track)
{
  DefineMaterial(track.GetMaterialCutsCouple());
...
G4double NewEnergyLossProcess::ContinuousStepLimit(const G4Track& track, 
...
G4double NewEnergyLossProcess::GetMeanFreePath(
                             const G4Track& track,
                             G4double,
                             G4ForceCondition* condition)
...
G4double NewEnergyLossProcess::GetContinuousStepLimit(
		const G4Track&,
...
G4PhysicsVector* 
NewEnergyLossProcess::LambdaPhysicsVector(const G4MaterialCutsCouple*, 
					  G4double)
{
  G4PhysicsVector* v = 
...
void NewEnergyLossProcess::AddCollaborativeProcess(
            NewEnergyLossProcess* p)
{
  G4bool add = true;
...
void 
NewEnergyLossProcess::SetDEDXTable(G4PhysicsTable* p, G4EmTableType tType)
{
  G4bool verb = false;
  if(fTotal == tType) {
    ...
    if(verb) {
      G4cout<< "NewEnergyLossProcess::SetDEDXTable "
	    << particle->GetParticleName()
    ...
    if(verb) {
      G4cout<< "NewEnergyLossProcess::SetIonisationTable "
	    << particle->GetParticleName()
            << " oldTable " << theDEDXTable << " newTable " << p 
            << " ion " << theIonisationTable 
	    << " IsMaster " << isMaster 
...
void NewEnergyLossProcess::SetCSDARangeTable(G4PhysicsTable* p)
{
  theCSDARangeTable = p; 
...
void NewEnergyLossProcess::SetRangeTableForLoss(G4PhysicsTable* p)
{
...
void NewEnergyLossProcess::SetSecondaryRangeTable(G4PhysicsTable* p)
{
...
void NewEnergyLossProcess::SetInverseRangeTable(G4PhysicsTable* p)
{
...
void NewEnergyLossProcess::SetLambdaTable(G4PhysicsTable* p)
{
  if(1 < verboseLevel) {
...
void NewEnergyLossProcess::SetSubLambdaTable(G4PhysicsTable* p)
{
  theSubLambdaTable = p;
  if(1 < verboseLevel) {
...
const G4Element* NewEnergyLossProcess::GetCurrentElement() const
{
...
void NewEnergyLossProcess::SetCrossSectionBiasingFactor(G4double f, 
							G4bool flag)
{
  if(f > 0.0) { 
    biasFactor = f; 
...
void 
NewEnergyLossProcess::ActivateForcedInteraction(G4double length, 
...
void 
NewEnergyLossProcess::ActivateSecondaryBiasing(const G4String& region, 
					       G4double factor, 
