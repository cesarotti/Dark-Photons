...
if [ "MEV" = "GEV" ];then
  if [ "CM" = "MM" ];then
    cmd9=`diff -q -b testStreamIOVarious.cout /media/sf_darkphotons/hepmc/test/testStreamIOVarious.output`
  fi
fi
  cmd=`diff -q -b testStreamIO.cout /media/sf_darkphotons/hepmc/test/testStreamIO.output`
...
if [ "MEV" = "GEV" ];then
  ...
  cmd=`diff -q -b testStreamIO.cout /media/sf_darkphotons/hepmc/test/testStreamIO.output`
  cmd3=`sed 's/e-00/e+00/g'  /media/sf_darkphotons/hepmc/test/testHepMCParticle.output | \
...
if [ "MEV" = "GEV" ];then
  ...
  cmd=`diff -q -b testStreamIO.cout /media/sf_darkphotons/hepmc/test/testStreamIO.output`
  cmd3=`diff -q -b testStreamIOParticle.out /media/sf_darkphotons/hepmc/test/testHepMCParticle.output`
  cmd4=`diff -q -b testStreamIOVarious.out testNaN.out`
esac
