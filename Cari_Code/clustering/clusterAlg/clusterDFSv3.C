	...
	for (VP_iterator=crysIDs.begin(); VP_iterator!=crysIDs.end(); ++VP_iterator)
	  {
	    ...
	    for(a=VVP_iterator->begin(); a!=VVP_iterator->end(); ++a)
	      {
		double energy =  (a->second)*(VP_iterator->second)/energyTot;
	        pair<int, double> hit(a->first, energy);
		if (hit.second>energyThres) {shower.push_back(hit);}
	      }
...
void clusterDFSv3() {
  ...
  for (int k=0; k<nEvents; k++)
      ...
      for (VVP_iterator=clusters.begin(); VVP_iterator!=clusters.end(); ++VVP_iterator)
	{
      ...
      vector<vector<pair<double, int> > > detectorE;
      int num = 9;
	
      for (VVP_iterator=detector.begin(); VVP_iterator!=detector.end(); ++VVP_iterator)
	{ 
	  vector<pair<double, int> > shower =  energySort(*VVP_iterator);
	  if (shower.size()>num) {shower.erase(shower.begin()+num, shower.end());}
	  detectorE.push_back(shower);
	  vector<pair<double, int> >::iterator c;
	  for (c=shower.begin(); c!=shower.end(); ++c)
	    {  
	      gammaH->Fill(IDtoX(c->second), IDtoY(c->second), c->first);
	    }
	  
	}
      
      vector<vector<pair<double, int> > >::iterator b;
      for (b=detectorE.begin(); b!=detectorE.end(); ++b)
	{
	  cout << "Photon energy: " << reconstruct(*b).first
	       << " MeV" << endl; 
	  cout << "Photon position: ("
	       << reconstruct(*b).second.first << ", " 
	       << reconstruct(*b).second.second << ")" << endl;
