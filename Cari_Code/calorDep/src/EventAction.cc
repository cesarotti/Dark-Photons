 ...
 = static_cast<TestHitsCollection*>(hce->GetHC(fHCID));
  ...
  for (G4int i=0; i<20; i++)
      ...
      if (eDep>0.)
	{
	  ...
	  if (eDep>0.5*MeV)
	    {
	      G4cout << "Hit in ring number : " << hit->GetCellID() << G4endl;
	      analysisMan->FillH1(0, hit->GetCellID());
	      analysisMan->FillNtupleDColumn(hit->GetCellID(), eDep);
	}

    }
    }
  analysisMan->FillNtupleIColumn(20, numHit);
  analysisMan->FillNtupleDColumn(21, energyDep);
