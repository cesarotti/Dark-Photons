...
{ 
  ...
  tree->SetBranchAddress("Energy", &energyTot);
  ...
  TH1D* energyReso = new TH1D("energyReso", "Energy_resolution",  1000, -1000, 1000);
  TH1D* posResoX = new TH1D("posResoX", "XPosition_resolution", 100, -100, 100); 
  TH1D* posResoY = new TH1D("posResoY", "YPosition_resolution", 100, -100, 100); 
  ...
  for (int i = 0; i < nEvents; i++)
      ...
      for (vv_iter=ordered.begin(); vv_iter!=ordered.end(); ++vv_iter)
	{
	  ...
	  energyReso->Fill(photon.first-energyTot);
	  posResoX->Fill((photon.second.first)*CRYStoMM-posX);
	  posResoY->Fill((photon.second.second)*CRYStoMM-posY);
	}
    }
