...
// ********************************************************************
//
// $Id: NewBremsstrahlungRelModel.cc 75582 2013-11-04 12:13:01Z gcosmo $
//
// -------------------------------------------------------------------
//
// GEANT4 Class file
//
//
// File name:     NewBremsstrahlungRelModel
//
...
const G4double 
NewBremsstrahlungRelModel::xgi[]={ 0.0199, 0.1017, 0.2372, 0.4083,
				   0.5917, 0.7628, 0.8983, 0.9801 };
const G4double 
NewBremsstrahlungRelModel::wgi[]={ 0.0506, 0.1112, 0.1569, 0.1813,
				   0.1813, 0.1569, 0.1112, 0.0506 };
const G4double 
NewBremsstrahlungRelModel::Fel_light[]  = {0., 5.31  , 4.79  , 4.74 ,  4.71};
const G4double 
NewBremsstrahlungRelModel::Finel_light[] = {0., 6.144 , 5.621 , 5.805 , 5.924};
    ...
    use_completescreening(false),isInitialised(false)
{
  ...
  SetHighestEnergy(2000*MeV);
  SetLowestTheta(1*deg);
  SetHighestTheta(6*deg);
...
void NewBremsstrahlungRelModel::InitialiseConstants()
{
  facFel = G4Log(184.15);
  facFinel = G4Log(1194.);
...
void NewBremsstrahlungRelModel::SetParticle(const G4ParticleDefinition* p)
{
  particle = p;
...
void NewBremsstrahlungRelModel::SetupForMaterial(const G4ParticleDefinition*,
...
void NewBremsstrahlungRelModel::Initialise(const G4ParticleDefinition* p,
...
void NewBremsstrahlungRelModel::InitialiseLocal(const G4ParticleDefinition*,
...
G4double 
NewBremsstrahlungRelModel::MinPrimaryEnergy(const G4Material*,
...
G4double NewBremsstrahlungRelModel::ComputeDEDXPerVolume(
...
G4double NewBremsstrahlungRelModel::ComputeBremLoss(G4double cut)
{
  G4double loss = 0.0;
...
G4double NewBremsstrahlungRelModel::ComputeCrossSectionPerAtom(
                                              const G4ParticleDefinition* p,
					      G4double kineticEnergy, 
					      G4double Z,   G4double,
					      G4double cutEnergy, 
...
G4double NewBremsstrahlungRelModel::ComputeXSectionPerAtom(G4double cut)
{
  G4double cross = 0.0;
...
void  NewBremsstrahlungRelModel::CalcLPMFunctions(G4double k)
{
...
G4double NewBremsstrahlungRelModel::ComputeRelDXSectionPerAtom(G4double gammaEnergy)
...
G4double NewBremsstrahlungRelModel::ComputeDXSectionPerAtom(G4double gammaEnergy)
// Relativistic model
...
void NewBremsstrahlungRelModel::SampleSecondaries(
				      ...
				      G4double maxEnergy)
{
  ...
  /*do {
    ...
    if ( f > fMax ) {
      G4cout << "### NewBremsstrahlungRelModel Warning: Majoranta exceeded! "
	     << f << " > " << fMax
