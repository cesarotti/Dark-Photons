...
void Sigma_sm_ll_a::setIdColAcol() 
{
  if(id1 == -11 && id2 == 11)
  {
    // Pick one of the flavor combinations (22,)
    int flavors[1][1] = {{22}}; 
    vector<double> probs; 
...
double Sigma_sm_ll_a::weightDecay(Event& process, int iResBeg, int iResEnd) 
{
...
void Sigma_sm_ll_a::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_0(w[1], w[0], w[2], pars->GC_3, amp[0]); 


}
double Sigma_sm_ll_a::matrix_1_epem_a() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 1; 
